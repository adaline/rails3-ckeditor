class CkeditorController < ApplicationController
  ...
  # GET /ckeditor/images
  def images
    if respond_to?(:current_user) and Ckeditor.file_manager_partition_users
      @images = Ckeditor.image_model.where(:user_id => current_user).order('id DESC')
    else
  ...
  def files
    if respond_to?(:current_user) and Ckeditor.file_manager_partition_users
      @files = Ckeditor.file_model.where(:user_id => current_user).order("id DESC")
    else
